...
const mockDonors: Donor[] = [
  ...
  },
  {
    ...
    age: 28,
    bloodGroup: 'A+',
    availableOrgans: ['cornea', 'bone_marrow'],
...
const mockRecipients: Recipient[] = [
  ...
  },
  {
    ...
    medicalHistory: 'Liver cirrhosis',
...
export const mockApi = {
  ...
  findMatches: async (recipientId: string): Promise<Match[]> => {
    ...
    const compatibleDonors = mockDonors.filter(donor => {
      ...
      const organAvailable = donor.availableOrgans.some(organ => 
        recipient.neededOrgans.includes(organ)
      );
      return bloodCompatible && organAvailable && donor.isActive;
    });
    ...
    return compatibleDonors.map((donor, index) => ({
      ...
      reason: `Blood type compatible, ${donor.availableOrgans.join(', ')} available`,
      status: 'pending',
  ...
  sendAlert: async (alert: Omit<Alert, 'id' | 'createdAt' | 'isActive'>): Promise<Alert> => {
    ...
    // Simulate alert delivery to compatible donors
    const compatibleDonors = mockDonors.filter(donor => {
      const bloodMatch = alert.targetBloodGroups.includes(donor.bloodGroup);
      const organMatch = donor.availableOrgans.some(organ => 
        alert.targetOrgans.includes(organ)
      );
...
function getCompatibilityReason(donor: Donor, recipient: Recipient): string {
  ...
  const sharedOrgans = donor.availableOrgans.filter(organ => 
    recipient.neededOrgans.includes(organ)
  );
  
  if (sharedOrgans.length > 0) {
    reasons.push(`${sharedOrgans.join(', ')} available`);
  }
