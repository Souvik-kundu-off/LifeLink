...
import { Donor, Recipient, BloodGroup, OrganType, UrgencyLevel } from '../../types';
...
export default function AlertSystem({ donors, recipients }: AlertSystemProps) {
  ...
  const calculateEstimatedReach = (formData: FormData) => {
    ...
    const selectedOrgans = organTypes.filter(organ => 
    ...
    const compatibleDonors = donors.filter(donor => {
      ...
      const organMatch = selectedOrgans.length === 0 || donor.availableOrgans.some(organ => 
      ...
      return bloodMatch && organMatch && donor.isActive && withinRadius;
    });
  ...
  const handleCreateAlert = async (formData: FormData) => {
    ...
    try {
      ...
      await mockApi.sendAlert(newAlert);
      setAlertSent(true);
                  ...
                  <SelectContent>
                    ...
                    {recipients.filter(r => r.status === 'waiting').map(recipient => (
                      <SelectItem key={recipient.id} value={recipient.id}>
                        {recipient.name} - {recipient.bloodGroup} - {recipient.neededOrgans.join(', ')}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input 
                    id="title" 
                    name="title" 
                    placeholder="e.g., Urgent: Kidney Donor Needed"
                    required 
                  />
                </div>
                <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-red-700 mt-1">
                        ...
                        <span>Needs: {recipient.neededOrgans.join(', ')}</span>
                      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {[
              ...
              {
                id: '2',
                title: 'Liver Transplant Required',
