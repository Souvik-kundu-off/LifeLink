...
export default function DonorMap({ donors, recipients }: DonorMapProps) {
  ...
  const filteredDonors = donors.filter(donor => {
    ...
    const organMatch = selectedOrgan === 'all' || !selectedOrgan || donor.availableOrgans.includes(selectedOrgan as OrganType);
    return bloodMatch && organMatch && donor.isActive;
  });

  const filteredRecipients = recipients.filter(recipient => {
    const bloodMatch = selectedBloodGroup === 'all' || !selectedBloodGroup || recipient.bloodGroup === selectedBloodGroup;
    const organMatch = selectedOrgan === 'all' || !selectedOrgan || recipient.neededOrgans.includes(selectedOrgan as OrganType);
    return bloodMatch && organMatch && recipient.status === 'waiting';
  });
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Organ/Tissue</label>
              <Select value={selectedOrgan} onValueChange={setSelectedOrgan}>
                ...
                <SelectContent>
                  ...
                  {organTypes.map(organ => (
                    <SelectItem key={organ} value={organ} className="capitalize">
                      {organ.replace('_', ' ')}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              filteredDonors.map((donor) => {
                ...
                return (
                  <div
                    ...
                    style={{ left: `${x}%`, top: `${y}%` }}
                    title={`${donor.name} - ${donor.bloodGroup} - ${donor.availableOrgans.join(', ')}`}
                  >
                    <div 
                    ...
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-lg shadow-lg p-3 text-xs whitespace-nowrap z-10">
                      ...
                      <div>Organs: {donor.availableOrgans.join(', ')}</div>
                    </div>
                  </div>
                );
              })
              ...
              filteredRecipients.map((recipient) => {
                ...
                return (
                  <div
                    ...
                    className="absolute transform -translate-x-1/2 -translate-y-1/2 cursor-pointer group"
                    style={{ left: `${x}%`, top: `${y}%` }}
                    title={`${recipient.name} - ${recipient.bloodGroup} - ${recipient.neededOrgans.join(', ')}`}
                  >
                    <div 
                    ...
                    <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white rounded-lg shadow-lg p-3 text-xs whitespace-nowrap z-10">
                      ...
                      <div>Needs: {recipient.neededOrgans.join(', ')}</div>
