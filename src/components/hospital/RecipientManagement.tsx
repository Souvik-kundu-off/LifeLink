...
export default function RecipientManagement({ recipients, onUpdate }: RecipientManagementProps) {
  ...
  const handleAddRecipient = async (formData: FormData) => {
    setLoading(true);
    try {
      const neededOrgans = organTypes.filter(organ => 
        formData.get(`organ_${organ}`) === 'on'
      );
      ...
      await mockApi.createRecipient(newRecipient);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Recipient Management</h2>
          <p className="text-gray-600">Register and manage patients waiting for organ transplants</p>
        </div>
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new patient waiting for organ transplant to the system.
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={(e) => {
              ...
              <div>
                <Label>Needed Organs/Tissues</Label>
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {organTypes.map(organ => (
                    <div key={organ} className="flex items-center space-x-2">
                      <Checkbox id={`organ_${organ}`} name={`organ_${organ}`} />
                      <Label htmlFor={`organ_${organ}`} className="capitalize">
                        {organ.replace('_', ' ')}
                      </Label>
                    </div>
                  ))}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage patient information and transplant status
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Needed Organs</TableHead>
                ...
                <TableRow key={recipient.id}>
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {recipient.neededOrgans.slice(0, 2).map(organ => (
                        <Badge key={organ} variant="secondary" className="text-xs">
                          {organ}
                        </Badge>
                      ))}
                      {recipient.neededOrgans.length > 2 && (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {selectedRecipient.neededOrgans.map(organ => (
