...
export default function DonorRecipientApp({ user, onLogout }: DonorRecipientAppProps) {
  ...
  const mockDonationRequests = [
    ...
    },
    {
      ...
      description: 'Patient waiting for kidney transplant, good match found'
    }
  ];
  ...
  const mockMyRequests = [
    {
      ...
      description: 'Planned surgery next month, need compatible blood'
    }
  ];
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={mockProfile.isAvailableToDonate ? 'default' : 'secondary'}>
                  {mockProfile.isAvailableToDonate ? 'Active' : 'Inactive'}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Total donations</span>
                <span className="font-semibold text-green-600">{mockProfile.totalDonations}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Last donation</span>
                <span className="text-sm">{mockProfile.lastDonation}</span>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold text-green-600">
                  {mockMyRequests.reduce((sum, req) => sum + req.responses, 0)}
                </span>
              </div>
              <div className="flex items-center justify-between">
          ...
          <CardContent>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary">{mockProfile.bloodGroup}</Badge>
              </div>
              <div className="flex items-center justify-between">
            ...
            {mockDonationRequests.map(request => (
              <div key={request.id} className="border rounded-lg p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">
                      {request.type === 'blood' ? `Blood ${request.bloodGroup}` : request.organType}
                    </span>
                  </div>
                  <Badge variant={
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  ...
                  <span>{request.distance} • {request.requestedAt}</span>
                </div>
              ...
              <div key={request.id} className="border rounded-lg p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">
                      {request.type === 'blood' ? `Blood ${request.bloodGroup}` : 'Organ Request'}
                    </span>
                  </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span>
                    {request.type === 'blood' ? `Blood Donation (${request.bloodGroup})` : `${request.organType} Donation`}
                  </span>
                </CardTitle>
                <Badge variant={
                  request.urgency === 'high' ? 'destructive' : 
  ...
  const renderRequestSection = () => (
    <div className="p-6 space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Request Donation Help</h2>
        <p className="text-gray-600">Create a request to find compatible donors in your area</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="cursor-pointer hover:shadow-lg transition-shadow">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <Heart className="h-5 w-5 text-blue-500" />
              <span>Request Organ Donation</span>
            </CardTitle>
            <CardDescription>
              Find compatible organ donors for transplant surgery
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2 text-sm text-gray-600 mb-4">
              ...
              <li>• Transplant team coordination</li>
              <li>• Priority-based matching</li>
            </ul>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage your active donation requests</CardDescription>
        </CardHeader>
        <CardContent>
          {mockMyRequests.length > 0 ? (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Configure your availability and preferences</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <span className="text-sm font-medium">Available organs:</span>
                <div className="flex flex-wrap gap-2">
                  {mockProfile.availableOrgans.map(organ => (
        ...
        <CardContent>
          <div className="grid md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">{mockProfile.totalDonations}</div>
