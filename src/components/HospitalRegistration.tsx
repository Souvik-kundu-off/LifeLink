...
export default function HospitalRegistration({ onLogin, onBack }: HospitalRegistrationProps) {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== 'hospital_staff') {
        throw new Error('Invalid credentials for hospital staff');
      }
      onLogin(user);
    } catch (err) {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Hospital registration submitted successfully! Your application is under review. You will receive confirmation within 2-3 business days.');
      setActiveTab('signin');
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="signin" className="space-y-4">
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <p className="text-sm text-gray-600 mb-3 font-medium">Try Demo Account:</p>
                  <Button
                    variant="outline"
                    size="sm"
                    className="w-full justify-between text-left hover:bg-blue-50"
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      <span className="text-sm">{demoLogin.email}</span>
                    </div>
